...
int main( int argc, char * argv[] )
{
  ...
  std::vector<double> deformationRegionSize; // {20, 20, 20}
	...
	typedef   itk::ImageFileWriter< MovingImageType >  DeformedWriterType;
	...
	DeformedWriterType::Pointer deformedWriter = DeformedWriterType::New();
	...
	typedef itk::ResampleImageFilter< MovingImageType, FixedImageType  >  FilterType;
  ...
  const double weightMultiplier=1.0/0.29622130393981934;
	...
	for( unsigned int n=0; n < numberOfNodes; n++ )
	{
		...
		//parameters[n]=radius*rand()/RAND_MAX-radius/2;                  // X coordinate
		//parameters[n+numberOfNodes]=radius*rand()/RAND_MAX-radius/2;    // Y coordinate
		//parameters[n+numberOfNodes*2]=radius*rand()/RAND_MAX-radius/2;  // Z coordinate
	} 
  ...
  for (int z=0; z<gridSize[2]; z++)
  {
    for (int y=0; y<gridSize[1]; y++)
    {
      ...
      {
        int paramIndex=(((z*gridSize[1])+y)*gridSize[0])+x;
        if ( abs(deformationPositionNodeIndices[0]-x)<SplineSupportRegionSize &&
          abs(deformationPositionNodeIndices[1]-y)<SplineSupportRegionSize &&
          abs(deformationPositionNodeIndices[2]-z)<SplineSupportRegionSize )
        {
          parameters[paramIndex+numberOfNodes*0]=deformationVector[0];
          parameters[paramIndex+numberOfNodes*1]=deformationVector[1];
          parameters[paramIndex+numberOfNodes*2]=deformationVector[2];
        }
        else
        {
          parameters[paramIndex+numberOfNodes*0]=0;
          parameters[paramIndex+numberOfNodes*1]=0;
          parameters[paramIndex+numberOfNodes*2]=0;
        }
      }
    }
  }
